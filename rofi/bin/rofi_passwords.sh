...
function main() {  
    ...
    local rofi_input 
    rofi_input="$(colored-pango-icon  "$WARNING_COLOR" ) Add password\n"

    if (( passwords_count > 0 )); then 
        rofi_input+="$(colored-pango-icon   "$WARNING_COLOR" ) Remove password\n"
        local -r remove_password_line=1
    fi

    if pass git status 2>/dev/null \
        && pass git remote -v 2>/dev/null | grep "origin" ; then 
        rofi_input+="$(colored-pango-icon  "$WARNING_COLOR" ) Sync from git\n"
        local -r sync_git_line=1
    fi

    local -r usb_drives=$(usb-available)
    if [ -n "${usb_drives}" ] && (( passwords_count > 0 )); then 
        rofi_input+="$(colored-pango-icon 󰕓 "$WARNING_COLOR" ) Sync to USB\n"
    ...
    if [ -n "$passwords" ]; then 
        ...
        do 
            if [ -n "$line" ]; then 
                rofi_input_passwords+="$(colored-pango-icon  ) $line\n"
            fi
        done <<< "$passwords"
    fi
    ...
    case "$variant" in 
        ...
        $(( idx_usb)) )
            ...
            do 
                if [ -n "$line" ]; then 
                    rofi_input_usb+="$(colored-pango-icon 󰕓 ) $line\n"
                fi
            done <<< "$usb_drives"
        ...
        *)
            ...
            if ! $otp_present; then
                ...
                local -r variant_password_type=$( \
                    echo -en "$(colored-pango-icon  ) Password\n$(colored-pango-icon 󰀠 ) One-time password\n" \
                    | rofi -config "$XDG_CONFIG_HOME/rofi/dmenu-single-column.rasi" \
                    -markup-rows -i -dmenu -no-custom \
                    -format 'i' \
                    -p " $selected_service" \
