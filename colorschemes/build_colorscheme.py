...
parser = argparse.ArgumentParser(description="Build colorscheme set for rofi, eww and alacritty",
...
def colorscheme_format(template, colorscheme_json):
    return template.format(
        ...
        black_variant =     mix_color(colorscheme_json["black"], colorscheme_json["white"], 0.8),
        red_variant =       mix_color(colorscheme_json["black"], colorscheme_json["red"], 0.3),
        green_variant =     mix_color(colorscheme_json["black"], colorscheme_json["green"], 0.3),
        yellow_variant=     mix_color(colorscheme_json["black"], colorscheme_json["yellow"], 0.3),
        blue_variant =      mix_color(colorscheme_json["black"], colorscheme_json["blue"], 0.3),
        magenta_variant =   mix_color(colorscheme_json["black"], colorscheme_json["magenta"], 0.3),
        cyan_variant =      mix_color(colorscheme_json["black"], colorscheme_json["cyan"], 0.3),
        white_variant =     mix_color(colorscheme_json["black"], colorscheme_json["white"], 0.3),
        
        bg_base =           colorscheme_json["background"],
        bg_view =           mix_color(colorscheme_json["background"], colorscheme_json["foreground"], 0.9),
        bg_popup =          mix_color(colorscheme_json["background"], colorscheme_json["foreground"], 0.8),
        bg_focused =        mix_color(colorscheme_json["background"], colorscheme_json["foreground"], 0.7),
        bg_selected =       mix_color(colorscheme_json["background"], colorscheme_json["foreground"], 0.6),
        ...
        color_on_accent =   mix_color(colorscheme_json[colorscheme_json["accent"]], colorscheme_json["background"], 0.2),
        color_on_warning =  mix_color(colorscheme_json[colorscheme_json["warning"]], colorscheme_json["background"], 0.2),
        color_on_error =    mix_color(colorscheme_json[colorscheme_json["error"]], colorscheme_json["background"], 0.2),
        color_on_success =  mix_color(colorscheme_json[colorscheme_json["success"]], colorscheme_json["background"], 0.2),
...
generaions = [{"source": "alacritty.template",  "result": "theme.alacritty.toml"},
...
for colorscheme in colorschemes_list:
    print("Generate {} colorscheme:".format(colorscheme))
    source_file = open(os.path.join("./.sources/palettes/", colorscheme), "r")
    ...
    for formated_theme in generaions:
        print("\tProcess {}...".format(formated_theme["source"]))

        theme_template = open(os.path.join("./.sources/templates/", formated_theme["source"]), "r").read()
        theme_data = colorscheme_format(theme_template, colorscheme_json)

        result_file = open(os.path.join(name, formated_theme["result"]), "w")
